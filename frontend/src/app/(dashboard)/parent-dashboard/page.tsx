...
import { BookOpen, Loader2, Sparkles, ChevronRight, Award, GraduationCap, Target } from 'lucide-react';
...
export default function ParentDashboard() {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (err.response?.status === 403) {
          setError(isAr ? 'غير مصرح: يسمح فقط لأولياء الأمور بالوصول إلى هذه الصفحة.' : 'Unauthorized: Only parents can access this dashboard.');
        } else {
          setError(isAr ? 'فشل تحميل بيانات لوحة التحكم. الرجاء المحاولة لاحقاً.' : 'Failed to load dashboard data. Please try again later.');
        }
      } finally {
        setIsLoading(false);
      }
    ...
    return (
      <div className="flex h-screen bg-background-light dark:bg-background-dark overflow-hidden">
        ...
        <main className="flex-1 flex justify-center items-center p-6">
          <div className="glass-panel p-8 text-center max-w-md w-full border border-red-500/20 bg-red-500/5 backdrop-blur-md rounded-2xl">
            <h2 className="text-xl font-bold text-red-400 mb-2">{isAr ? 'تم رفض الوصول' : 'Access Denied'}</h2>
  ...
  const studentName = student_info?.name || (isAr ? 'طالبك' : 'your student');
  const studentLevel = student_info?.level || (isAr ? 'غير محدد' : 'Unassigned');

  return (
    <div className="flex h-screen bg-background-light dark:bg-background-dark overflow-hidden">
      <Sidebar />
      
      <main className="flex-1 flex flex-col p-6 lg:p-10 overflow-y-auto hide-scrollbar text-start">
        {/* Header Section */}
        <header className="mb-8">
          ...
          >
            {isAr 
              ? `أهلاً بك مجدداً. إليك تقدم الطالب ${studentName}.`
              : `Welcome back. Here is ${studentName}'s progress.`
            }
          </motion.h1>
          <p className="text-slate-500 dark:text-slate-400">
            {t.monitoringLevel.replace('{level}', studentLevel)}
          </p>
        </header>
        ...
        <motion.div
          ...
          className="mb-10 rounded-3xl overflow-hidden shadow-2xl border border-white/5 min-h-[250px] md:min-h-[350px] w-full block bg-slate-900 relative"
        >
          <NewsCarousel />
        </motion.div>
